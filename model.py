...
class GaussianSplatting2D(nn.Module):
    ...
    def _init_gaussians(self, args):
        ...
        self.beta = nn.Parameter(
            torch.full((self.num_gaussians, 1), self.beta_value, dtype=self.dtype, device=self.device), 
            requires_grad=True
        )
    ...
    def _get_masked_ssim_tensor(self, img1, img2, window_size=11):
        ...
        # Creiamo una "Valid Map": consideriamo validi solo i pixel dove 
        # la finestra 11x11 cade COMPLETAMENTE (o quasi) dentro la maschera (es. > 0.9)
